...
const CGFloat CSArrowSide = 15.0f;

@implementation CSBannerFooter
...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        self.state = CSBannerFooterStateIdle;
    }
    return self;
}
...
- (void)layoutSubviews
{
    ...
    CGFloat arrowX = self.bounds.size.width / 2 - CSArrowSide - 2;
    CGFloat arrowY = self.bounds.size.height / 2 - CSArrowSide / 2;
    CGFloat arrowW = CSArrowSide;
    CGFloat arrowH = CSArrowSide;
    self.arrowView.frame = CGRectMake(arrowX, arrowY, arrowW, arrowH);
    
    CGFloat labelX = self.bounds.size.width / 2 + 2;
    CGFloat labelY = 0;
    CGFloat labelW = CSArrowSide;
...
- (void)setState:(CSBannerFooterState)state
{
    _state = state;
    
    switch (state) {
        case CSBannerFooterStateIdle:
        {
            ...
            break;
        case CSBannerFooterStateTrigger:
        {
...
- (void)setIdleTitle:(NSString *)idleTitle
{
    ...
    if (self.state == CSBannerFooterStateIdle) {
...
- (void)setTriggerTitle:(NSString *)triggerTitle
{
    ...
    if (self.state == CSBannerFooterStateTrigger) {
