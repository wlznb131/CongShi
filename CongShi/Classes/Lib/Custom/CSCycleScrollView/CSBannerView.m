...
#import "CSBannerCell.h"

#define CS_TOTAL_ITEMS (self.itemCount * 20000)
#define CS_FOOTER_WIDTH 64.0
#define CS_PAGE_CONTROL_HEIGHT 32.0

@interface CSBannerView() <UICollectionViewDataSource,UICollectionViewDelegate>

@property (nonatomic, strong) UICollectionView *collectionView;
@property (nonatomic, strong) UICollectionViewFlowLayout *flowLayout;

@property (nonatomic, strong) CSBannerFooter *footer;
...
- (void)updateSubviewsFrame
{
    ...
    self.flowLayout.footerReferenceSize = CGSizeMake(CS_FOOTER_WIDTH, self.frame.size.height);
    ...
    if (CGRectEqualToRect(self.pageControl.frame, CGRectZero)) {
        ...
        CGFloat h = CS_PAGE_CONTROL_HEIGHT;
        CGFloat x = 0;
...
- (void)fixDefaultPosition
{
    ...
    if (self.shouldLoop) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [self.collectionView scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:(CS_TOTAL_ITEMS / 2) inSection:0]
...
{
    CSBannerCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:banner_item forIndexPath:indexPath];
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)theCollectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)theIndexPath
{
    ...
    if(kind == UICollectionElementKindSectionFooter)
    {
        ...
        self.footer = (CSBannerFooter *)footer;

        // 配置footer的提示语
        if ([self.dataSource respondsToSelector:@selector(banner:titleForFooterWithState:)]) {
            self.footer.idleTitle = [self.dataSource banner:self titleForFooterWithState:CSBannerFooterStateIdle];
            self.footer.triggerTitle = [self.dataSource banner:self titleForFooterWithState:CSBannerFooterStateTrigger];
        }
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    {
        // 开始出现footer
        if (footerDisplayOffset > CS_FOOTER_WIDTH) {
            if (lastOffset > 0) return;
            self.footer.state = CSBannerFooterStateTrigger;
        } else {
            if (lastOffset < 0) return;
            self.footer.state = CSBannerFooterStateIdle;
        }
        lastOffset = footerDisplayOffset - CS_FOOTER_WIDTH;
    }
}
 ...
 *  数据源
 */
- (void)setDataSource:(id<CSBannerViewDataSource>)dataSource
{
...
- (UICollectionView *)collectionView
{
    if (!_collectionView) {
        ...
        // 注册cell
        [_collectionView registerClass:[CSBannerCell class] forCellWithReuseIdentifier:banner_item];
        
        // 注册 \ 配置footer
        [_collectionView registerClass:[CSBannerFooter class] forSupplementaryViewOfKind:UICollectionElementKindSectionFooter withReuseIdentifier:banner_footer];
        _collectionView.contentInset = UIEdgeInsetsMake(0, 0, 0, -CS_FOOTER_WIDTH);
    }
