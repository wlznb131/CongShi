...
#import "CSSegmentPageViewController.h"

const void* _CSSEGMENTPAGE_CURRNTPAGE_SCROLLVIEWOFFSET = &_CSSEGMENTPAGE_CURRNTPAGE_SCROLLVIEWOFFSET;

@interface CSSegmentPageViewController ()

@property (nonatomic, strong) UIView<CSSegmentPageControllerHeaderProtocol> *headerView;
@property (nonatomic, strong) NSMutableArray *controllers;
@property (nonatomic, assign) CGFloat segmentToInset;
@property (nonatomic, weak) UIViewController<CSSegmentControllerDelegate> *currentDisplayController;
@property (nonatomic, strong) NSLayoutConstraint *headerHeightConstraint;

@end

@implementation CSSegmentPageViewController

#pragma mark - life cycle methods

-(instancetype)initWithControllers:(UIViewController<CSSegmentControllerDelegate> *)controller, ...
{
    self = [super init];
    if (self) {
        NSAssert(controller != nil, @"the first controller must not be nil!");
        [self _setUp];
        UIViewController<CSSegmentControllerDelegate> *eachController;
...
{
    return [[CSSegmentPageHeader alloc] init];
}
...
-(void)_baseConfigs
{
    ...
    self.segmentView = [[CSSegmentView alloc] init];
    ...
    [self.controllers enumerateObjectsUsingBlock:^(UIViewController<CSSegmentControllerDelegate> *controller, NSUInteger idx, BOOL *stop) {
    ...
    UIViewController<CSSegmentControllerDelegate> *controller = self.controllers[0];
...
-(void)_layoutControllerWithController:(UIViewController<CSSegmentControllerDelegate> *)pageController
{
...
-(UIScrollView *)scrollViewInPageController:(UIViewController <CSSegmentControllerDelegate> *)controller
{
...
-(void)addObserverForPageController:(UIViewController <CSSegmentControllerDelegate> *)controller
{
    UIScrollView *scrollView = [self scrollViewInPageController:controller];
    if (scrollView != nil) {
        [scrollView addObserver:self forKeyPath:NSStringFromSelector(@selector(contentOffset)) options:NSKeyValueObservingOptionNew context:&_CSSEGMENTPAGE_CURRNTPAGE_SCROLLVIEWOFFSET];
    }
}

-(void)removeObseverForPageController:(UIViewController <CSSegmentControllerDelegate> *)controller
{
    UIScrollView *scrollView = [self scrollViewInPageController:controller];
    if (scrollView != nil) {
        @try {
...
{
    if (context == _CSSEGMENTPAGE_CURRNTPAGE_SCROLLVIEWOFFSET) {
...
-(void)segmentControlDidChangedValue:(UISegmentedControl *)sender
{
    ...
    UIViewController<CSSegmentControllerDelegate> *controller = self.controllers[index];
